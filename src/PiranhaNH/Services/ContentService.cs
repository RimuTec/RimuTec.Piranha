...
namespace RimuTec.PiranhaNH.Services
{
        ...
        where TModelBase : ContentBase
    {
        ...
        public IList<Block> TransformBlocks(IEnumerable<BlockEntity> blocks)
        {
            ...
            foreach (var block in blocks)
            {
                ...
                if (blockType != null)
                {
                    var model = (Block)Activator.CreateInstance(blockType.Type);
                    model.Id = block.Id;
                    ...
                    {
                        var parent = models.Find(m => m.Id == block.ParentId.Value);

                        if (parent != null && typeof(BlockGroup).IsAssignableFrom(parent.GetType()))
                        {
                            ((BlockGroup)parent).Items.Add(model);
                        }
                        else
                        {
        ...
        public IList<BlockEntity> TransformBlocks(IList<Block> models)
        {
            ...
            if (models != null)
            {
                for (var n = 0; n < models.Count; n++)
                {
                    ...
                    if (type != null)
                    {
                        ...
                        {
                            var blockItems = TransformBlocks(((BlockGroup)models[n]).Items);
        ...
        private IEnumerable GetEnumerable<T>(T model, string regionId) where T : ContentBase
        {
         object value;
        ...
        private object GetRegion<T>(T model, string regionId) where T : ContentBase
        {
            if (model is IDynamicContent dynamicModel)
            {
        ...
        private bool HasRegion<T>(T model, string regionId) where T : ContentBase
        {
            if (model is IDynamicContent dynamicModel)
            {
        ...
        private IList<Guid> MapRegion(TContent content, object region, RegionType regionType, string regionId, int sortOrder = 0)
        {
            var items = new List<Guid>();
                ...
                if (fieldType != null)
                {
                    ...
                    if (fieldValue != null)
                    {
                        ...
                        if (field == null)
                        {
                            ...
                            //field.Id = Guid.NewGuid(); // Guid assigned by NHibernate
        ...
        private void SetSimpleValue<T>(T model, string regionId, TField field) where T : ContentBase
        {
            if (model is IDynamicContent dynamicModel)
            {
        ...
        /// <param name="regionId">The region id</param>
        /// <param name="field">The field</param>
        private void AddSimpleValue<T>(T model, string regionId, TField field) where T : ContentBase
        {
            if (model is IDynamicContent dynamicModel)
            {
        ...
        /// <param name="field">The field</param>
        private void SetComplexValue<T>(T model, string regionId, string fieldId, TField field) where T : ContentBase
        {
            if (model is IDynamicContent dynamicModel)
            {
